...
async fn send_message(pool: &Pool<Sqlite>, from: &E164, body: &str) -> Result<String> {
    process_message(
        pool,
        SmsMessage {
            From: from.to_string(),
...
async fn test_new_user_registration(pool: Pool<Sqlite>) -> Result<()> {
    ...
    let response = send_message(&pool, &TEST_PHONE, "hi").await?;
    assert!(response.contains("Greetings!"));
    assert!(response.contains("name"));

    // Test invalid name command
    let response = send_message(&pool, &TEST_PHONE, "name").await?;
    ...
    let response = send_message(&pool, &TEST_PHONE, "name John Doe").await?;
    assert!(response.contains("Hello, John Doe!"));

    // Verify user was created in database
    let user = query_as!(User, "SELECT * FROM users WHERE number = ?", **TEST_PHONE)
        .fetch_one(&pool)
        .await?;
    assert_eq!(user.name, "John Doe");
    assert_eq!(user.number, **TEST_PHONE);
...
async fn test_help_and_info_commands(pool: Pool<Sqlite>) -> Result<()> {
    ...
    // Register user first
    send_message(&pool, &TEST_PHONE, "name John Doe").await?;

    // Test help command
    let response = send_message(&pool, &TEST_PHONE, "h").await?;
    ...
    let response = send_message(&pool, &TEST_PHONE, "info").await?;
    assert!(response.contains("Reply \"info X\""));

    // Test info with valid command
    let response = send_message(&pool, &TEST_PHONE, "info name").await?;
    assert!(response.contains("set your preferred name"));

    // Test info with invalid command
    let response = send_message(&pool, &TEST_PHONE, "info invalidcommand").await?;
...
async fn test_contact_management(pool: Pool<Sqlite>) -> Result<()> {
    ...
    let response = send_message(&pool, &TEST_PHONE, "contacts").await?;
    ...
    let result = process_vcard(&pool, TEST_PHONE.to_owned(), vcard).await?;
    ...
    let response = send_message(&pool, &TEST_PHONE, "delete Alice").await?;
    assert!(response.contains("Found these contacts"));
    assert!(response.contains("Alice Smith"));

    // Test contact deletion - confirm
    let response = send_message(&pool, &TEST_PHONE, "confirm 1").await?;
...
async fn test_user_deletion(pool: Pool<Sqlite>) -> Result<()> {
    ...
    let mut reader = ical::VcardParser::new(vcard_data.as_bytes());
    let vcard = reader.next().unwrap();
    process_vcard(&pool, TEST_PHONE.to_owned(), vcard).await?;

    // Test stop command - should cascade delete contacts due to foreign key constraint
    let response = send_message(&pool, &TEST_PHONE, "stop").await?;
...
async fn test_contact_updates(pool: Pool<Sqlite>) -> Result<()> {
    ...
    let result = process_vcard(&pool, TEST_PHONE.to_owned(), vcard1).await?;
    assert!(matches!(result, ImportResult::Added));
    ...
    let result = process_vcard(&pool, TEST_PHONE.to_owned(), vcard2).await?;
...
async fn test_multi_number_contact_selection(pool: Pool<Sqlite>) -> Result<()> {
    ...
    let response = send_message(&pool, &TEST_PHONE, "confirm 1b, 2a").await?;
    ...
    let response = send_message(&pool, &TEST_PHONE, "confirm 1c").await?; // Invalid letter
    assert!(response.contains("Invalid letter selection: c"));

    let response = send_message(&pool, &TEST_PHONE, "confirm 2a").await?; // Invalid contact number
    assert!(response.contains("Contact number 2 not found"));

    let response = send_message(&pool, &TEST_PHONE, "confirm abc").await?; // Invalid format
...
async fn test_group_deletion(pool: Pool<Sqlite>) -> Result<()> {
    ...
    process_vcard(&pool, TEST_PHONE.to_owned(), reader.next().unwrap()).await?;
    ...
    // Create first group
    let response = send_message(&pool, &TEST_PHONE, "group Alice, Bob").await?;
    assert!(response.contains("Found these contacts"));
    let response = send_message(&pool, &TEST_PHONE, "confirm 1,2").await?;
    assert!(response.contains("Created group \"group0\""));

    // Create second group
    let response = send_message(&pool, &TEST_PHONE, "group Bob, Carol").await?;
    assert!(response.contains("Found these contacts"));
    let response = send_message(&pool, &TEST_PHONE, "confirm 1,2").await?;
    ...
    let response = send_message(&pool, &TEST_PHONE, "delete group0").await?;
    ...
    let response = send_message(&pool, &TEST_PHONE, "delete group").await?;
    assert!(response.contains("Found these groups:"));
    assert!(response.contains("group1 (2 members)"));
    let response = send_message(&pool, &TEST_PHONE, "confirm 1").await?;
    assert!(response.contains("Deleted 1 group"));
    ...
    let response = send_message(&pool, &TEST_PHONE, "delete nonexistent").await?;
    assert!(response.contains("No groups or contacts found"));

    // Test separate deletion of group and contact
    // First recreate a group
    let _response = send_message(&pool, &TEST_PHONE, "group Alice, Bob").await?;
    let response = send_message(&pool, &TEST_PHONE, "confirm 1,2").await?;
    assert!(response.contains("Created group \"group0\""));
